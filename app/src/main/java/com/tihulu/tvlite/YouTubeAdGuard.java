...
final class YouTubeAdGuard {
    ...
    private static final String SCRIPT =
            ...
            + "}return v;};"
            + ""
            + "const hookGlobal=(name)=>{"
            + "try{"
            + "let stored;"
            + "Object.defineProperty(window,name,{"
            + "configurable:true,enumerable:true,"
            + "get:function(){return stored;},"
            + "set:function(v){stored=prune(v,0);}"
            + "});"
            + "}catch(e){}"
            + "};"
            + "hookGlobal('ytInitialPlayerResponse');"
            + "hookGlobal('ytInitialData');"
            + "try{if(window.ytInitialPlayerResponse)prune(window.ytInitialPlayerResponse,0);}catch(e){}"
            + "try{if(window.ytInitialData)prune(window.ytInitialData,0);}catch(e){}"
            + ""
            + "const cleanText=(t)=>{"
