...
public final class TvBraveActivity extends ChromeTabbedActivity
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        // Fullscreen D-pad mode keeps native player directions/back. Select is deliberately routed
        // through our short/long press tracker so short OK is replayed to the player while long OK
        // can still switch into Cursor mode even when a video is already fullscreen.
        if (mHtmlFullscreen
                && mNavigationMode == TvNavigationMode.DPAD
                && !isSelectKey(event.getKeyCode())) {
    ...
    private boolean handleSelectKeyEvent(KeyEvent event) {
        ...
        long heldMs = Math.max(0L, event.getEventTime() - downEvent.getEventTime());
        boolean wasLongPress = mSelectLongPressConsumed;
        mSelectLongPressConsumed = false;
        if (wasLongPress) return true;

        // A busy renderer/main thread can delay the 550 ms callback until after key-up. The
        // KeyEvent timestamps are generated by Android's input dispatcher, so use them as a
        // release-time fallback. A long OK still toggles exactly once and never also clicks.
        if (heldMs >= SELECT_LONG_PRESS_MS) {
    ...
    private void handleSelectLongPress() {
        if (mPendingSelectDownEvent == null || mSelectLongPressConsumed) return;
    ...
    private View getPointerTarget() {
        ...
        // Tab.getView() is the active visible surface: a NativePage view for Brave/Chromium home
        // and new-tab pages, and the ContentView for ordinary web pages. Prefer it so native home
        // controls receive Android touch fallback instead of sending mouse events to a hidden web
        // content view. Keep getContentView() only as a defensive fallback during tab transitions.
        View activeView = tab.getView();
        return activeView != null ? activeView : tab.getContentView();
    }
