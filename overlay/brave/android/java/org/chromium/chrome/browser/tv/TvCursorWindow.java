 ...
 * above the activity's ordinary ViewOverlay. A tiny cursor drawn in that overlay therefore works
 * on native pages but can disappear as soon as real web content is shown. Keep one transparent,
 * non-focusable application window instead. It is created lazily only when Cursor mode is first
 * requested, never receives touch/key input, and is dismissed when Cursor mode is disabled.
 */
